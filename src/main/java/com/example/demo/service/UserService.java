...
public class UserService implements UserDetailsService {
    ...
    public User setUserDifficultyLevel(String userId, Integer difficultyLevel) {
        ...
        // UserService와 UserCategoryService는 같은 애플리케이션 내의 컴포넌트이므로 직접 조회 가능
        log.info("🔄 [KAFKA] Preparing to publish difficulty change event: userId={}, difficulty={}", userId, difficultyLevel);
        Map<String, List<String>> currentCategories = getUserCategoriesFromCacheOrRepository(userId);
        log.info("📤 [KAFKA] Publishing difficulty change event: userId={}, difficulty={}, categories={}", 
                userId, difficultyLevel, currentCategories);
        kafkaProducerService.publishDifficultyChangeEvent(userId, difficultyLevel, currentCategories);
        log.info("✅ [KAFKA] Difficulty change event published successfully: userId={}", userId);
    ...
    public User resetUserDifficultyLevel(String userId) {
        ...
        // 카프카 이벤트 발행 (UserService 내부에서 카테고리 정보 조회하여 함께 발행)
        // UserService와 UserCategoryService는 같은 애플리케이션 내의 컴포넌트이므로 직접 조회 가능
        log.info("🔄 [KAFKA] Preparing to publish difficulty reset event: userId={}, difficulty=2", userId);
        Map<String, List<String>> currentCategories = getUserCategoriesFromCacheOrRepository(userId);
        log.info("📤 [KAFKA] Publishing difficulty reset event: userId={}, difficulty=2, categories={}", 
                userId, currentCategories);
        kafkaProducerService.publishDifficultyChangeEvent(userId, 2, currentCategories);
        log.info("✅ [KAFKA] Difficulty reset event published successfully: userId={}", userId);
