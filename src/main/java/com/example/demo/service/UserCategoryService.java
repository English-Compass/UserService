...
public class UserCategoryService {
    ...
    public List<UserCategory> saveUserCategories(String userId, Map<String, List<String>> categories) {
        ...
        log.info("🔄 [KAFKA] Preparing to publish category change event: userId={}", userId);
        Integer currentDifficulty = userService.getUserDifficultyLevel(userId);
        // 난이도가 null이면 기본값 2 사용 (중급)
        if (currentDifficulty == null) {
            log.warn("⚠️ [KAFKA] User difficulty is null, using default value 2: userId={}", userId);
            currentDifficulty = 2;
        }
        log.info("📤 [KAFKA] Publishing category change event: userId={}, difficulty={}, categories={}", 
                userId, currentDifficulty, categories);
        kafkaProducerService.publishCategoryChangeEvent(userId, categories, currentDifficulty);
        log.info("✅ [KAFKA] Category change event published successfully: userId={}", userId);
